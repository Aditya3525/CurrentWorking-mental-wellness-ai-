...
export function EnhancedContentLibrary({ onNavigate, user }: ContentLibraryProps) {
  ...
  const fetchRecommended = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRecommended(data?.data?.recommendations || []);
        setPagination(prev => ({ ...prev, page: 1, total: (data?.data?.recommendations || []).length, pages: 1 }));
      } else {
  ...
  const openContent = (item: ContentItem) => {
    // Prefer externalUrl, then fileUrl. For articles without URLs, fallback to alert preview.
    if (item.externalUrl) {
    ...
    // No URL available; could navigate to a detail page if implemented
    // For now, show a simple message
    alert('Opening content: ' + item.title);
  };
  ...
  <Button className="w-full" size="sm" onClick={() => openContent(item)}>
  ...
  <Button className="w-full" size="sm" onClick={() => onNavigate('library') /* placeholder for playlist open */}>
          <PlayCircle className="h-4 w-4 mr-2" />
          View Playlist
        </Button>
      </CardContent>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
            ...
            <div className="ml-auto flex gap-2">
              ...
              {user?.approach && (
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    setSelectedApproach(user.approach!);
                    setCurrentPage(1);
                  }}
                >
                  Tailored to me
      ...
      <div className="max-w-6xl mx-auto p-6">
        {/* (Tabs list removed as requested) */}
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-4">
          
          <TabsContent value="content" className="space-y-6">
            {/* Filters */}
            <div className="rounded-md border bg-white/70 backdrop-blur p-3 shadow-sm space-y-2">
        ...
        {activeTab === 'recommended' && (
          <div className="space-y-6 mt-6">
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {recommended.map(renderContentCard)}
              </div>
            )}
            {!loading && recommended.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-4">Complete an assessment or chat with the assistant to personalize suggestions.</p>
              </div>
            )}
          </div>
        )}

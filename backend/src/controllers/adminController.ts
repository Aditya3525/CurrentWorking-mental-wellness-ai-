...
const contentSchema = Joi.object({
  ...
  // Content is required for articles; for other types it's optional and may be empty
  ...
  difficulty: Joi.string().valid('Beginner', 'Intermediate', 'Advanced').empty('').optional(),
  tags: Joi.string().trim().allow('', null).optional(),
  fileType: Joi.string().trim().allow('', null).optional(),
  ...
  severityLevel: Joi.string().valid('Mild', 'Moderate', 'Severe').empty('').optional(),
  targetAudience: Joi.string().trim().allow('', null).optional(),
  effectiveness: Joi.number().min(0).max(5).allow(null).optional(),
  prerequisites: Joi.string().trim().allow('', null).optional(),
  outcomes: Joi.string().trim().allow('', null).optional(),
...
export const createContent = async (req: any, res: Response) => {
  try {
    ...
    // If non-article type and content is missing, auto-fill a placeholder
    if (normalized.type && normalized.type !== 'article' && !normalized.content) {
      normalized.content = '';
    }

    // For video/audio types, enforce at least one URL present
    if ((normalized.type === 'video' || normalized.type === 'audio') && !normalized.externalUrl && !normalized.fileUrl) {
      res.status(400).json({ success: false, error: 'For video/audio, provide externalUrl or fileUrl.' });
      return;
    }

    const { error, value } = contentSchema.validate(normalized, {
      abortEarly: false,
      allowUnknown: true,
      stripUnknown: true,
    });
...
export const updateContent = async (req: any, res: Response) => {
  try {
    ...
    if (normalized.type && normalized.type !== 'article' && normalized.content === undefined) {
      // leave as-is if not provided; if explicitly empty string it was set to null above
    }
    if ((normalized.type === 'video' || normalized.type === 'audio') && normalized.externalUrl == null && normalized.fileUrl == null) {
      // We'll still validate; but proactively inform
    }
    const { error, value } = contentSchema.validate(normalized, {
      abortEarly: false,
      allowUnknown: true,
    ...
    const { id: _omitId, createdAt: _omitCreatedAt, updatedAt: _omitUpdatedAt, viewCount: _omitViewCount, rating: _omitRating, ratingCount: _omitRatingCount, ...updatable } = value as any;
...
export const updatePlaylist = async (req: any, res: Response) => {
  try {
    ...
    const { id: _pid, createdAt: _pc, updatedAt: _pu, viewCount: _pv, rating: _pr, ratingCount: _prc, ...playlistUpdatable } = value as any;
